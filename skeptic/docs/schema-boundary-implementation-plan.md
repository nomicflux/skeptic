...
- Updated immediate compatibility dependents that were still using the mixed bridge behavior, including `analysis/bridge/algebra.clj`, `analysis/bridge/render.clj`, `analysis/normalize.clj`, `analysis/schema.clj`, `analysis/schema/cast_support.clj`, `analysis/schema/map_cast.clj`, `analysis/schema/map_ops.clj`, `analysis/schema/value_check.clj`, and `inconsistence/report.clj`.
...
- A message regression in unexpected-map-key reporting was fixed after the Step 1 cut so visible-path errors still say `is not allowed by the expected schema` or `is not allowed by the declared schema`.
...
- Repointed direct callers to `skeptic.analysis.type_algebra`, including `analysis/schema/cast_support.clj`, `analysis/schema/cast_kernel.clj`, and `test/skeptic/analysis/schema/type_helpers_test.clj`.
...
- Focused `lein test` runs passed for `skeptic.analysis.schema.type-helpers-test` and `skeptic.analysis.schema.cast-test` after the migration.
...
- Repointed schema-facing display callers to `skeptic.analysis.bridge.canonicalize/schema-display-form`, including `src/skeptic/schematize.clj` and the schema branch in `src/skeptic/inconsistence/display.clj`.
...
- Updated the direct regression coverage to the type-only contract in `test/skeptic/analysis/schema/type_helpers_test.clj`, `test/skeptic/analysis/calls_test.clj`, `test/skeptic/analysis/annotate_test.clj`, `test/skeptic/checking/pipeline_test.clj`, `test/skeptic/inconsistence/report_test.clj`, `test/skeptic/analysis/origin_test.clj`, and related projection helpers.
- Focused `lein test` runs passed for `skeptic.inconsistence.display-test`, `skeptic.inconsistence.report-test`, `skeptic.analysis.schema.type-helpers-test`, `skeptic.analysis.normalize-test`, `skeptic.analysis.calls-test`, `skeptic.analysis.annotate-test`, `skeptic.analysis.origin-test`, `skeptic.checking.pipeline-test`, `skeptic.core-test`, and `skeptic.schematize-test`.

## Step 4: Add typed declaration ingestion

Completed.

- Kept the existing raw schema APIs in `src/skeptic/schematize.clj` unchanged. `get-fn-schemas*`, `get-fn-schemas`, `collect-schemas`, `var-schema-desc`, and `ns-schemas` still remain schema-facing and return `SchemaDesc`.
- Added schema-boundary typed ingestion helpers in `src/skeptic/schematize.clj`: `schema-desc->typed-entry`, `fn-schema->typed-entry`, `one->typed-arg-entry`, `arglist->typed-entry`, and `typed-ns-schemas`.
- Made `schema-desc->typed-entry` the typed declaration boundary adapter. It imports declaration payloads through `schema->type` and returns semantic-only entries shaped around `:name`, `:type`, optional `:output-type`, and optional typed `:arglists`.
...
- Added focused regression coverage in `test/skeptic/schematize_test.clj` for callable typed entries, non-callable typed entries, semantic-only output shape, varargs typing, and dynamic `Any` fallback entries returned by `typed-ns-schemas`.
...
- Switched namespace declaration ingestion to `typed-ns-schemas` in the live checking path, including `src/skeptic/core.clj`, and updated the active analysis and checking tests to build declaration dictionaries from typed entries instead of raw `SchemaDesc`.
...
- Moved the recursive cast dispatch from `src/skeptic/analysis/schema.clj` into `skeptic.analysis.cast/check-cast`, with a semantic-type-only contract based on `type_ops` normalization and no schema import inside the engine.
- Reduced `src/skeptic/analysis/schema.clj` to the schema-boundary adapter surface. `check-cast` and `schema-compatible?` now convert raw Schema inputs once and delegate to the type-only engine.
- Repointed cast-support users to `skeptic.analysis.cast.support`, including `src/skeptic/analysis/schema/map_ops.clj`, `src/skeptic/analysis/schema/value_check.clj`, `src/skeptic/analysis/schema/valued.clj`, `test/skeptic/analysis/schema/cast_test.clj`, `test/skeptic/analysis/schema/map_cast_test.clj`, `test/skeptic/analysis/schema/type_helpers_test.clj`, and `test/skeptic/analysis/calls_test.clj`.
- Removed the old schema-cast implementation files `src/skeptic/analysis/schema/cast_support.clj`, `src/skeptic/analysis/schema/cast_kernel.clj`, and `src/skeptic/analysis/schema/map_cast.clj` so non-schema code no longer depends on them.
- Deleted the unused payload definitions `WithPlaceholder`, `ArgCount`, and `AnnotatedExpression` from `src/skeptic/analysis/schema.clj`.
- Focused `lein test` runs passed for `skeptic.analysis.schema.cast-test`, `skeptic.analysis.schema.map-cast-test`, `skeptic.inconsistence.path-test`, `skeptic.inconsistence.report-test`, `skeptic.analysis.schema.type-helpers-test`, and `skeptic.analysis.calls-test` after the migration.
...
- `src/skeptic/core.clj` already used `schematize/typed-ns-schemas` from earlier steps and needed no further structural change for this cut.
...
Reporting, path rendering, and checking consume semantic types all the way through, while schema-facing wording such as `declared return schema` remains unchanged for user-visible diagnostics.
...
- Schema-boundary tests remain schema-facing where intended, including `test/skeptic/schematize_test.clj`, `test/skeptic/analysis/schema/cast_test.clj`, `test/skeptic/analysis/schema/map_cast_test.clj`, and the schema canonicalization / placeholder tests in `test/skeptic/analysis/schema/type_helpers_test.clj`.
...
- `skeptic.analysis.schema/check-cast` remains as an explicit schema-boundary adapter for raw Schema callers.
- `schematize/typed-ns-schemas` becomes the only declaration ingress used by internal analysis code.
