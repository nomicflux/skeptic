...
(deftest constructors-attach-provenance
  (testing "every constructor attaches its prov arg"
    ...
    (is (= p-schema (prov/of (sut/->InfCycleT p-schema))))
...
(deftest metadata-invisible-to-equality
  (testing "two types with same body but different prov are = (metadata doesn't affect =)"
    (is (= (sut/->GroundT p-schema :int 'Int)
           (sut/->GroundT p-native :int 'Int))))
  (testing "different bodies are not equal even with same prov"
    (is (not= (sut/->GroundT p-schema :int 'Int)
