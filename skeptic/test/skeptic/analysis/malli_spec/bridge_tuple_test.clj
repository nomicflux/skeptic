...
(deftest tuple-with-single-member
  (is (= (at/->SeqT tp (at/pattern-from-prefix-tail [(at/->GroundT tp :int 'Int)] nil) :vector)
...
(deftest tuple-with-unknown-leaf-falls-back-to-dyn-slot
  (let [result (sut/malli-spec->type tp [:tuple :int :uuid])]
    (is (= (at/->SeqT tp (at/pattern-from-prefix-tail [(at/->GroundT tp :int 'Int) (at/Dyn tp)] nil) :vector)
           result))))
