...
(deftest closed-union-type-exhaustion
  (let [union (ato/union-type tp [(T s/Str) (T s/Int)])]
    (is (sut/exhausted-by-types? union [(T s/Str) (T s/Int)]))
    (is (not (sut/exhausted-by-types? union [(T s/Str)])))))

(deftest unknown-union-is-open
  (let [union (ato/union-type tp [(T s/Str) (at/Dyn tp)])]
    (is (not (sut/sum-type? union)))
    (is (not (sut/exhausted-by-types? union [(T s/Str) (at/Dyn tp)])))))
