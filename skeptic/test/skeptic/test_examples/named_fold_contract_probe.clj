...
(s/defn produce-either
  :- (s/either IntBranch StrBranch)
...
(s/defn produce-cond-pre
  :- (s/cond-pre IntBranch StrBranch)
...
(s/defn visible-maybe-branch-mismatch :- s/Int
  []
  {:result (produce-maybe)})

(s/defn produce-constrained
  :- (s/constrained IntBranch pos?)
  []
  1)

(s/defn visible-constrained-branch-mismatch :- s/Int
  []
  {:result (produce-constrained)})
